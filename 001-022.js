...
//44. Ваша задача — вернуть объем чашки, зная диаметр верха, диаметр дна и высоту.
// Вы знаете, что существует устойчивый градиент сверху вниз.
// Вы хотите вернуть объем, округленный до двух десятичных знаков.
// Примеры:
// cupVolume(1, 1, 1);
// //returns 0.79
function cupVolume(d1, d2, height) {
	return (1 / 3 * Math.PI * height * ((d1 / 2) ** 2) + d1 * d2 + ((d2 / 2) ** 2))
}

// 45. задача найти объем конуса и вернуть значение огругленное до целого
function volume(r, h) {
	return Math.floor((h * Math.PI * r * r) / 3)
}

// 46. Завершите функцию, которая преобразует шестнадцатеричное число (заданное в виде строки) в десятичное число.
function hexToDec(hexString) {
	return parseInt(hexString, 16);
}

//47.В этом ката вам нужно просто определить, является ли данный год високосным или нет. Если вы не знаете правил, вот они:
